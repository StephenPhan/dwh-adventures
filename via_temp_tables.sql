--The following table creates a pivot which outputs the estimated ranking of an app_id in a given country's rank list.
		...
		--To calculate GP or iOS we must comment in/out two WHERE clauses.
...
select * 
into ##topthousandprunesp
from ##topthousandtempsp
where (rank_BR<1001
	OR rank_CA<1001
	OR rank_CN<1001
	OR rank_DE<1001
...
SELECT DISTINCT app_id, app_store_id
...
into ##appsdktempsp
from (
		SELECT DISTINCT app_table.app_id, app_table.app_store_id, sdk_table.sdk_id, sdk_name, sdk_present
		from ##topthousandprunesp pr
			left join dw_stage.apptopia.app as app_table on pr.app_id = app_table.app_id
...
SELECT distinct app_id, app_store_id, price, publisher_name, app_initial_release_date
...
into ##informationtempsp
from(
        SELECT distinct a.app_id, /*a.category_id,*/ a.app_store_id, a.publisher_id, p.publisher_name, /*a.subcategory_id,*/ a.app_initial_release_date
        , row_number() over(partition by a.app_id order by a.dwloaddate desc) as rn
        , case when app_price_cents = 0 then 'free' else 'paid' end as price
        from ##topthousandprunesp pr
			left join (select app_id, app_store_id, publisher_id, app_initial_release_date, app_price_cents, dwloaddate from dw_stage.apptopia.app) as a on pr.app_id = a.app_id
...
select distinct a.app_id, a.app_kind, BR, CA, CN, DE, FI, FR, GB, JP, KR, MX, RU, SE, US, 
rank_BR, rank_CA, rank_CN, rank_DE, rank_FI, rank_FR, rank_GB, rank_JP, rank_KR, rank_MX, rank_RU, rank_SE, rank_US,
b.app_store_id, case when sdk_table.app_id is not null then 'found' else 'missing' end as missingSDK,
unity, unreal, cocos, marmalade, corona, corona_labs, xamarin,
price, publisher_name, app_initial_release_date
from ##topthousandprunesp a
left join ##appsdktempsp b on a.app_id = b.app_id
left join ##informationtempsp c on a.app_id = c.app_id
left join (select distinct app_id from ##appsdktempsp) sdk_table on a.app_id = sdk_table.app_id
